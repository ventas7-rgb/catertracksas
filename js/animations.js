document.addEventListener('DOMContentLoaded', () => {
  ...
  const categories = [
    ['tren-de-rodaje', 'Tren de rodaje'], ['repuestos-ipd', 'IPD'], ['repuestos-kmp', 'KMP'],
    ['herramientas-de-corte', 'Herramientas de corte'], ['inyeccion', 'Inyección'], ['tren-de-fuerza', 'Tren de fuerza'],
    ['partes-hidraulicas', 'Hidráulica'], ['ajuste', 'Ajuste'], ['lubricantes', 'Lubricantes'],
    ['monitores', 'Monitores'], ['llantas', 'Llantas'], ['miscelaneos', 'Misceláneos']
  ];
  const catalogData = {
    'repuestos-ipd': ['Repuestos IPD', 'Anillos de motor', 'Camisas de cilindro', 'Pistones de motor', ['Motor', 'Empaques', 'Otros']],
    'repuestos-kmp': ['Repuestos KMP', 'Kit de sellos', 'Bomba de agua', 'Juego de empaques', ['Motor', 'Sellos', 'Otros']],
    'herramientas-de-corte': ['Herramientas de corte', 'Cuchillas', 'Puntas de excavadora', 'Adaptadores de diente', ['Cuchillas', 'Puntas', 'Adaptadores']],
    'inyeccion': ['Inyección', 'Inyector diésel', 'Bomba de inyección', 'Boquillas de inyección', ['Inyectores', 'Bombas', 'Boquillas']],
    'tren-de-fuerza': ['Tren de fuerza', 'Discos de transmisión', 'Convertidor de torque', 'Engranajes de mando final', ['Transmisión', 'Convertidores', 'Mando final']],
    'partes-hidraulicas': ['Partes hidráulicas', 'Bomba hidráulica', 'Mangueras hidráulicas', 'Válvula de control', ['Bombas', 'Mangueras', 'Válvulas']],
    'ajuste': ['Ajuste', 'Kit de tornillos', 'Bujes de desgaste', 'Pasadores de maquinaria', ['Tornillos', 'Bujes', 'Pasadores']],
    'lubricantes': ['Lubricantes', 'Aceite para motor diésel', 'Filtro de aceite', 'Grasa multipropósito', ['Aceites', 'Filtros', 'Grasas']],
    'monitores': ['Monitores', 'Monitor de maquinaria', 'Módulo electrónico', 'Alternador industrial', ['Monitores', 'Módulos', 'Alternadores']],
    'llantas': ['Llantas', 'Llanta para cargador', 'Neumático OTR', 'Cámara para maquinaria', ['Llantas', 'Neumáticos', 'Cámaras']],
    'miscelaneos': ['Misceláneos', 'Espejos para maquinaria', 'Luces de trabajo LED', 'Accesorios de cabina', ['Cabina', 'Iluminación', 'Accesorios']]
  };
  ...
  const filterProducts = (subcategory) => {
    const visibleProducts = productCards.filter((card) => subcategory === 'all' || card.dataset.subcategory === subcategory);
    productCards.forEach((card) => card.toggleAttribute('hidden', !visibleProducts.includes(card)));
    if (catalogResult) {
  ...
  if (catalogPage && pageData && !catalogPage.querySelector('.product-card')) {
    const [title, firstProduct, secondProduct, thirdProduct, subcategories] = pageData;
    ...
    const generatedFilters = [...catalogPage.querySelectorAll('.subcategory-filter')];
    ...
    const updateGeneratedCatalog = (subcategory) => {
      const visibleCards = generatedCards.filter((card) => subcategory === 'all' || card.dataset.subcategory === subcategory);
      generatedCards.forEach((card) => card.toggleAttribute('hidden', !visibleCards.includes(card)));
      generatedResult.textContent = `${visibleCards.length} ${visibleCards.length === 1 ? 'producto disponible' : 'productos disponibles'}`;
    };
    generatedFilters.forEach((filter) => filter.addEventListener('click', () => {
  ...
  if (header && !document.querySelector('.catalog-navigation')) {
    ...
    categoryNavigation.innerHTML = `<div class="container"><span class="catalog-navigation__label">Catálogo</span><ul><li><a href="${pageCategory ? '../' : ''}index.html">Inicio</a></li>${categories.map(([slug, label]) => `<li><a href="${pageCategory ? '../' : ''}${slug}/index.html"${slug === pageCategory ? ' aria-current="page"' : ''}>${label}</a></li>`).join('')}</ul></div>`;
